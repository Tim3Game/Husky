...
import { HuskyCommand, HuskyUtil } from '../../../lib';
...
export default class extends HuskyCommand {
    ...
    public async info(message: KlasaMessage): Promise<void> {
        ...
        const infoEmbed = new MessageEmbed()
            ...
            .addField(
                'Uso',
                message.language.get('COMANDO_WELCOMEIMAGE_USO', [settings.get('prefix')]),
            );
    ...
    public async channel(message: KlasaMessage, [name]: [string]): Promise<KlasaMessage> {
        ...
        if (channel instanceof TextChannel) {
            ...
            await settings
                .update('welcome_image.channel', channel.id)
                .then(() =>
                    message
                        .sendLocale('COMANDO_WELCOMEIMAGE_SUB_CHANNEL_ACTUALIZADO', [
                            channel.toString(),
                        ])
    ...
    public async message(message: KlasaMessage, [...value]: [string]): Promise<unknown> {
        if (!value)
            return message
                .sendLocale('COMANDO_WELCOMEIMAGE_SUB_NO_MSG')
                .then((msg) => msg.delete({ timeout: 10000 }));
        const { settings } = message.guild;
        await settings
            .update('welcome_image.message', value.join(' '))
            .then(() =>
                message
                    .sendLocale('COMANDO_WELCOMEIMAGE_SUB_MSG_ACTUALIZADO')
                    .then((msg) => msg.delete({ timeout: 10000 })),
            );
    }
    ...
    public async background(message: KlasaMessage, [url]: [string]): Promise<void> {
        if (
            ...
            await settings
                .update('welcome_image.background', url)
                .then(() =>
                    message
                        .sendLocale('COMANDO_WELCOMEIMAGE_SUB_BACKGROUND_ACTUALIZADO')
                        .then((msg) => msg.delete({ timeout: 10000 })),
                );
        } else {
            message
                .sendLocale('COMANDO_WELCOMEIMAGE_SUB_BACKGROUND_NO_URL')
                .then((msg) => msg.delete({ timeout: 10000 }));
        }
    }
    ...
    public async color(message: KlasaMessage, [prop, color]: [string, string]): Promise<unknown> {
        ...
        if (!this.checkProp(prop))
            return message
                .sendLocale('COMANDO_WELCOMEIMAGE_SUB_COLOR_SIN_PROPIEDAD')
                .then((msg) => msg.delete({ timeout: 10000 }));
        if (!this.checkColor(color))
            return message
                .sendLocale('COMANDO_WELCOMEIMAGE_SUB_COLOR_SIN_COLOR')
                .then((msg) => msg.delete({ timeout: 10000 }));
        if (message.deletable) await message.delete().catch(() => null);
        // eslint-disable-next-line max-len
        await settings
            .update(`welcome_image.${this.checkProp(prop)}`, this.checkColor(color))
            .then(() =>
                message
                    .sendLocale('COMANDO_WELCOMEIMAGE_SUB_COLOR_ACTUALIZADO')
                    .then((msg) => msg.delete({ timeout: 10000 })),
            );
    }
    public async on(message: KlasaMessage): Promise<void> {
        ...
        if (message.deletable) await message.delete().catch(() => null);
        await settings
            .update('welcome_image.enabled', true)
            .then(() =>
                message
                    .sendLocale('COMANDO_WELCOMEIMAGE_ACTIVADO')
                    .then((msg) => msg.delete({ timeout: 10000 })),
            );
    }
    ...
    public async off(message: KlasaMessage): Promise<void> {
        ...
        if (message.deletable) await message.delete().catch(() => null);
        await settings
            .update('welcome_image.enabled', false)
            .then(() =>
                message
                    .send('COMANDO_WELCOMEIMAGE_DESACTIVADO')
                    .then((msg) => msg.delete({ timeout: 10000 })),
            );
    }
